public class Table {
	...
	public int checkingFreehours(String[][] table, String eidikotita, int duration) {// βάση ημερομηνίας
							...
							if (pl == totalcells) {
								...
								if (pl % 2 == 0) {
									...
									if (tempint == 17) {
										...
										System.out.println(table[i][0] + " - " + table[i + totalcells][0]);
										exists = j;
										break;
									}
								} else {
	...
	public int checkingFreehours(String[][] table, String eidikotita, int duration, String docname) {// βάση ΚΑΙ γιατρού

		int thesi = 0;
		int exists = 0;
		for (int j = 1; j <= Employees.employees.size(); j++) {
			if ((eidikotita == Employees.employees.get(j).getSpecialty()) && (docname == table[0][j])) {
				thesi = j;
				break;
			}
		...
		for (int i = 1; i <= 16; i++) {
			...
			if (duration <= 30) {
				if (table[i][thesi] == " OXI ") {
					System.out.println(table[i][0]);
					exists = thesi;
					// για να έφτασε εδώ σημαίνει ότι έχει βει ΤΟΝ γιατρο με κενό την ώρα
					// table[i][0]
					// άρα βγαίνω τώρα απο αυτην την ζώνη ώρας θέλω να αλλάξει τώρα το i για να
					// εκτυπώσω και
